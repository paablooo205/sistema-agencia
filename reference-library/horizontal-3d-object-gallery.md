...
- Animation type: Rotación 3D continua por objeto + desplazamiento horizontal de la galería — combina `HorizontalGallery` (pendiente en `component-library/README.md`) con el patrón de rotación de `ThreeScene`, aplicado a varios objetos a la vez en vez de uno solo protagonista.
...
- Useful for: Si un proyecto necesita mostrar varios productos 3D a la vez (no uno solo como `ProductReveal`/`ThreeScene`) — sería una extensión razonable de `ThreeScene` combinada con `HorizontalGallery` (ambos ya previstos en `component-library/README.md`) en vez de un componente nuevo desde cero.
- Avoid when: Solo hay un producto protagonista (ahí `ProductReveal`/`ThreeScene` ya cubren el caso, no hace falta el patrón de galería) o el proyecto no tiene varios modelos 3D reales disponibles.
